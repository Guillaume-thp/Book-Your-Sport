...
class ScrapUrbanSoccer
  def perform(city, time, duration, date)
    ...
    # duration format is different between lefive and urban
    if duration == '01:00'
      duration = '60'
    ...
    if Date.strptime(date.to_s).month != Date.current.month

      browser.span(class: ['DayPicker-NavButton', 'DayPicker-NavButton--next']).click
    end
    play_date = browser.div(text: Date.strptime(date).day.to_s) # Choisir le jour. Faire en dernier sinon bug sur chrome
    play_date.click
  ...
  def self.urban_array
    ['Puteaux', 'Meudon', 'Orsay', 'Asnières', "Porte d'aubervilliers", 'Evry-Courcouronnes', 'La Défense', "Porte d'Ivry", 'Guyancourt', 'Marne la Vallée']
  end
